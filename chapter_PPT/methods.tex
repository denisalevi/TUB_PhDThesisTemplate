...
For the results shown in Fig~\ref{fig:PPT_mechanism}E and \deleted{Supplementary} Fig~\ref{fig:PPT_mechanism}\added{F} we used a single integrate-and-fire  model neuron that received excitatory synaptic input. The membrane potential $V(t)$ %(in millivolts)
...
\end{align}
with $\tau_{\rm STDP}=20$~ms. $L(\Delta t)$ is the learning window of STDP \cite{Song2000}. Hard upper and lower bounds were imposed on the synaptic weights, such that $0\le g_i \le \bar{g}_{\rm max}$ for all $i$, where the dimensionless maximum synaptic weight was $\bar{g}_{\rm max}=0.006$. Parameters \replaced{$A^+=\eta \cdot \bar{g}_{\rm max}$}{ $A^+=0.005 \cdot \bar{g}_{\rm max}$ }
and $A^-=1.05 \cdot A^+$
