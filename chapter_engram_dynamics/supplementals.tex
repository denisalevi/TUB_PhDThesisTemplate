...
\begin{figure}[H]
  \centering
  % 6.5in is the width set in the Google Slides page setup
  \centerwidegraphic[width=6.5in]{figures/fig5_supplementary.pdf}
...
\begin{equation}
\l(i\frac{d}{dt} - \mb{H}\r)\mb{G}(t, t_0) =  \bd{\delta}(t-t_0) \,.
...
%    \label{eq:greenfunc_qm}
    \Tilde{\mb{G}}(\omega) &=
    \lim_{\epsilon\to 0}\l[\omega - \mb{H} + i\epsilon\r]^{-1} \,.
...
\begin{equation}
    G_{\mb{e}_\mu} = \mb{e}_\mu^\dagger \mb{G} \mb{e}_\mu =  \sum_{ij} e^*_i e_j \bd{\psi}_i^\dagger \mb{G} \psi_j = \sum_i \l|e_i\r|^2 G_i \,.
\end{equation}
...
\begin{align}
   \mc{F} \l[  S_{\mb{e}_\mu}(t) \r] = \mc{S}_{\mb{e}_\mu}(\omega) \propto
    \text{Im}  \l[  G_{\mb{e}_\mu}(\omega) \r] \,.
\end{align}
...
\end{equation}
defined in the interval $\l|E\r|<R = 2\sigma_H \sqrt{N}= 2\sigma_0$.
...
\begin{align*}
    G_{\l<\mb{e}_\mu\r>} (\omega) & = \l< \mb{e}_\mu^\dagger \mb{G}(\omega) \mb{e}_\mu \r> =
    \sum_i \l< \l|c_i\r|^2 \r> \frac{1}{\omega - E_i+ i \epsilon} \\
    &
...
\begin{equation*}
    G_{\l<\mb{e}_\mu\r>} (\omega)  \approx \hspace{0.1cm} \int dE \frac{\rho_H(E)}{w-E+ i \epsilon}.
\end{equation*}
This integral corresponds to the Stieltjes transformation of the semicircle law. It can be solved performing a geometric series expansion of $\rho_H(E)$, and interchanging summation and integration. Alternative solutions make use of analytical continuation and contour integration. We refer to \cite{jiang2021wigner} and to \cite{bai2010spectral} for detailed calculations.
...
$$
\mc{S}(\omega)=\lim_{\epsilon\to0^+} \frac{1}{\pi} \text{Im}\l[ G_{\l<\mb{e}_\mu\r>} (\omega) \r] =
\rho_H(w)
$$
...
\begin{equation}
    S(t-t') = \mc{F}^{-1}\l[ \mc{S}(\omega \r)] = \mc{F}^{-1}\l[ \rho_H(\omega) \r] =
    \frac{1}{2 \pi}\int_{-R}^{R} d\omega \rho_H(\omega) e^{-i\omega(t-t')} = 2 \frac{J_1\l(R|t-t'|\r)}{R|t-t'|}
...
\begin{figure}%[H]
    \centering
    \centerwidegraphic[width=\linewidth]{figures/supplementary_full_engram.pdf}
    \caption{ \small
...
$$
\Tilde{\mb{G}}(\omega) = \l[\omega - \mb{H} \r]^{-1}
$$
...
\begin{equation}
    \mb{G}^s(\omega) = \l[ \omega - \mb{H}_s - \mb{V} \l(\omega-\mb{H}_b\r)^{-1}\mb{V}^\dagger \r]^{-1} =
     \l[\l(\mb{G}^{s_0}\r)^{-1} - \bd{\Sigma}(\omega) \r]^{-1}
     = \l(\bd{\Lambda}(\omega)\r)^{-1} \,.
...
\begin{align}
\bd{\Lambda}(\omega) = \l(\mb{G}^{s_0}\r)^{-1} - \bd{\Sigma}(\omega)
& =
\mb{U}_s^{-1} \l(\Tilde{\mb{G}}^{s_0}\r)^{-1} \mb{U}_s -
\mb{V} \mb{U}_b^{-1} \l(\omega-\hat{\mb{H}}_b\r)^{-1} \mb{U}_b \mb{V}^\dagger
\\
& = \mb{U}_s^{-1} \l[ \l(\Tilde{\mb{G}}^{s_0}\r)^{-1} - \hat{\mb{V}} \l(\omega-\hat{\mb{H}}_b\r)^{-1} \hat{\mb{V}}^\dagger \r] \mb{U}_s \,,
\end{align}
...
\begin{align}
    ...
    & = \l(\Tilde{\mb{G}}^0_s\r)^{-1}_{ii} - \sum_{jk} \hat{\mb{V}}_{ij}(\omega-\hat{\mb{H}}_b)_{jk} \hat{\mb{V}}^\dagger_{ki}
    \\
    &=
    \omega - E^s_i - \sum_j \Tilde{\mb{V}}_{ij} \Tilde{\mb{V}}^\dagger_{ji} (\omega-E^b_j)^{-1}
    \\
    &=
    \omega - E^s_i - \sum_j \l| \Tilde{\mb{V}}_{ij} \r|^2 (\omega-E^b_j)^{-1}
...
\begin{align}
    ...
    \omega - E^s_i - \sum_j \l< \l| \Tilde{\mb{V}}_{ij} \r|^2 \r> (\omega-E^b_j)^{-1}
    \\
    &
    \Tilde{\bd{\Lambda}}_{ik} (\omega) \approx
    \sum_j \l< \Tilde{\mb{V}}_{ij} \Tilde{\mb{V}}^*_{kj} \r> (\omega-E^b_j)^{-1} \,.
\end{align}
The two averages read
\begin{align}
     \l< \l| \Tilde{\mb{V}}_{ij} \r|^2 \r> & =
    \sum_{kl} \l< \l| \mb{V}_{kl} \r|^2 \r>
    \l| (\mb{U}_s)_{ik} \r|^2
    \l| (\mb{U}_b^{-1})_{lj} \r|^2
    \\
    & =
    \frac{\sigma_0^2}{N}
    \sum_{k}
    ...
    \frac{\sigma_0^2}{N}
    \\
    \l< \Tilde{\mb{V}}_{ij} \Tilde{\mb{V}}^*_{kj} \r>
    & =
    \frac{\sigma_0^2}{N}
    \sum_l
...
\begin{equation}
    G^s_i(\omega) = \bd{\psi}_i^{s\dagger} \mb{G}_s(\omega) \bd{\psi}^s_i =  \l(\Tilde{\bd{\Lambda}}_{ii} (\omega) \r)^{-1} \approx
    \l[ \omega - E^s_i + i\epsilon - \frac{\sigma_0^2}{N} \sum_{j=1}^{N_b} (\omega-E^b_j)^{-1} \r]^{-1}
...
\begin{align}
    ...
    \\[0.2cm]
    & =
    \sigma_0^2 \frac{N_b}{N}\l(\frac{ \omega \mp \sqrt{\omega-R_b^2} }{R_b^2/2} \r) =
    \sigma_0^2 \frac{N_b}{N}\l(\frac{ \omega \mp \sqrt{\omega-R_b^2} }{2 \sigma_0^2 N_b/N } \r)
    \\[0.2cm]
    & = \frac{ \omega \mp \sqrt{\omega-R_b^2} }{2} \,,
\end{align}
where the $\mp$ is taken such that $\Sigma(\omega) \sim \frac{1}{\l|\omega\r|}$ for $\omega\to\infty$.
...
\begin{align}
    \Delta(\omega) &= \text{Re}\l[ \Sigma(\omega)\r] =
    \frac{1}{2} \l(\omega - \text{sign}(\omega)\sqrt{\omega^2-R_b^2}   \r)
    \label{eq:selfenergy_real}
    \\[0.2cm]
    \Gamma(\omega) &= \text{Im}\l[ \Sigma(\omega)\r] =
    \frac{\sqrt{R_b^2-\omega^2}}{2}
    = \frac{\pi R_b^2}{4} \rho_b(\omega) = \pi \sigma_0^2 \frac{N_b}{N} \rho_b(\omega)
    \label{eq:selfenergy_imag} \,.
\end{align}
Note that the second term in $\Delta(\omega)$ vanishes for $\l|\omega\r|<R_b$, and viceversa the imaginary part $\Gamma(\omega)$ vanishes for $\l|\omega\r|>R_b$.
...
\begin{equation}
    \mc{S}_i(\omega) =
    \frac{1}{\pi}\text{Im}\l[ G^s_i(\omega) \r] =
    \lim_{\epsilon\to 0} \l(
    \frac{1}{\pi}\frac{\Gamma(\omega)-\epsilon}{\big(\omega-E^s_i - \Delta(\omega)\big)^2 + \big(\Gamma(\omega)^2-\epsilon\big)^2}  \r) \,.
    \label{eq:spectralfunction_limit}
\end{equation}
We study this limit in two different cases, namely for
$\Gamma(\omega)\neq 0$ and $\Gamma(\omega) = 0$. These are given by $\l|\omega\r|<R_b$ and $\l|\omega\r|>R_b$ respectively, and are indicated by $_<$ and $_>$ subscripts.
In the first case, the spectral function reads:
\begin{equation}
    \mc{S}_i(\omega)_< =
    \frac{1}{\pi}\frac{\Gamma(\omega)}{\big(\omega-E^s_i - \Delta(\omega)\big)^2 + \Gamma(\omega)^2}
    \hspace{2cm} \text{for} \hspace{0.3cm} \l|\omega\r|<R_b \,.
    \label{eq:spectral<}
\end{equation}
...
If $\mc{E}(\omega)$ vanishes at $\l|\omega\r|>R_b$, the limit in Eq.\ref{eq:spectral>} gives a finite contribution. If, instead, the zero of $\mc{E}(\omega)$ lies in $[-R_b, R_b]$, the limit for $\mc{S}_i(\omega)_>$ is zero, and the only contribution comes from $\mc{S}_i(\omega)_<$.
...
\begin{equation}
    ...
    \frac{1}{\pi}\frac{-\epsilon}{\big(\omega-E^s_i - \Delta(\omega)\big)^2 + \epsilon^2}
    \approx
    -\delta(\omega-E_i) = -\delta\l(\omega-E_i^s-\frac{R_b^2}{4E_i^s}\r)
\end{equation}

The spectral function, which was peaked at one frequency when no coupling was present, takes now finite values at all frequencies $\l|\omega\r|<R_b$.
...
\begin{figure}%[H]
    \centering
    \centerwidegraphic[width=0.94\linewidth]{figures/supplementary_subsampling.pdf}
...
\mb{H}_{i<j}$ are independent random variables with identical normal distribution $\mc{N}\l(0, \frac{\sigma_0^2}{N}\r)$. All the remaining entries are fixed by anti-symmetry.
